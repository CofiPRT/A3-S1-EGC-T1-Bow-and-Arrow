...
BadBalloon::BadBalloon(glm::vec2 position) {
	this->position = position;
}

BadBalloon::~BadBalloon() {}

std::vector<RenderData> BadBalloon::GetRenderData() {
	...
	// BAD BALLOON BODY
	{
		std::vector<VertexFormat> vertices = {
			VertexFormat(glm::vec3(0), BAD_BALLOON_COLOR_BODY)
		};
		...
		for (int i = 0; i <= BALLOON_BODY_TRIANGLE_COUNT; i++) {
			...
			vertices.push_back(VertexFormat(glm::vec3(x, y, 0), BAD_BALLOON_COLOR_BODY));
		}
		...
		std::string name = BAD_BALLOON_MESH_BODY;
	...
	// BAD BALLOON SHINE
	{
		...
		for (int i = 0; i <= BALLOON_BODY_TRIANGLE_COUNT; i++) {
			...
			vertices.push_back(VertexFormat(glm::vec3(innerX, innerY, 0), BAD_BALLOON_COLOR_BODY));

			float percentageFactor = 1.0 - abs(1.0 - t / midAngle);
			float currPercent = BALLOON_SHINE_START_PERCENT + (double)percentageFactor * percentRange;

			float outerX = BALLOON_WIDTH / 2 * currPercent * cos(t);
			float outerY = BALLOON_HEIGHT / 2 * currPercent * sin(t);

			vertices.push_back(VertexFormat(glm::vec3(outerX, outerY, 0), BAD_BALLOON_COLOR_SHINE));
		}
		...
		std::string name = BAD_BALLOON_MESH_SHINE;
	...
	// BAD BALLOON KNOT
	{
		...
		std::vector<VertexFormat> vertices = {
			VertexFormat(glm::vec3(0,	maxY, 0), BAD_BALLOON_COLOR_KNOT),
			VertexFormat(glm::vec3(-x,	minY, 0), BAD_BALLOON_COLOR_KNOT),
			VertexFormat(glm::vec3(x,	minY, 0), BAD_BALLOON_COLOR_KNOT),
		};
		...
		std::string name = BAD_BALLOON_MESH_KNOT;
	...
	// BAD BALLOON STRING
	{
		...
		for (int i = 0; i < points.size(); i++) {
			...
			vertices.push_back(VertexFormat(vertex, BAD_BALLOON_COLOR_STRING));
		}
		...
		std::string name = BAD_BALLOON_MESH_STRING;
...
std::vector<std::string> BadBalloon::GetMeshNames() {
	
	std::vector<std::string> meshNames;

	if (IsAlive()) {
		meshNames.push_back(BAD_BALLOON_MESH_SHINE);
		meshNames.push_back(BAD_BALLOON_MESH_BODY);
	}

	meshNames.push_back(BAD_BALLOON_MESH_KNOT);
	meshNames.push_back(BAD_BALLOON_MESH_STRING);

	return meshNames;

}

int BadBalloon::GetScore() {
	return BAD_BALLOON_SCORE;
}